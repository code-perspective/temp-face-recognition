...
2. Chunk concatenated linear.weight [embedding_dim, N*backbone_dim] → N×[embedding_dim, backbone_dim] per-patch weights
...
def load_cryptoface_checkpoint(model, checkpoint_path: Union[str, Path]) -> None:
    ...
    model.eval()
    print("[weight_loader] Checkpoint loaded.", flush=True)
